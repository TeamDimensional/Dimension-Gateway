...
for (def t in ["wall", "fence", "stairs", "slab"])
    for (def c in ["green", "white"])
        mainItems.add(mitem("block_testifi_bucks_${c}_${t}"))
for (def i in 0..15) mainItems.add(mitem("block_colored_lamp", i))
for (def i in 0..9) mainItems.add(mitem("item_potion_ring", i))
for (def a in armor)
    for (def c in armor_colors)
        mainItems.add(mitem("item_${a}_crystal_${c}"))
...
for (def it in ["face", "_placer", "_liquiface", "_energyface", "_redstoneface", "_breaker"])
    empoweringItems.add(mitem("block_phantom${it}"))

for (def i in 0..5) {
    mainItems.add(mitem("block_crystal", i))
    empoweringItems.add(mitem("block_crystal_empowered", i))
    mainItems.add(mitem("item_crystal", i))
...
TooltipEvents.setTooltip(item("actuallyadditions:block_leaf_generator"), "tooltip.gateway.hint.leaf_generator")
